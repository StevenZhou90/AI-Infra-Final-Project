...
from scripts.build_pi0fast_empirical_vocab import (
    build_vocab_payload,
...
def test_empirical_vocab_expands_only_eligible_token_neighborhoods() -> None:
    assert expand_token_ids([3, 100], radius=2, min_token_id=50, max_token_id=102) == [3, 98, 99, 100, 101, 102]
...
def test_empirical_vocab_payload_can_expand_action_neighborhoods(tmp_path) -> None:
    rows = [
        {
            "token_ids": torch.tensor([7, 100, 100]),
            "mode": "target_eos",
        }
    ]
    torch.save(rows, tmp_path / "shard_00000.pt")
    ...
    payload = build_vocab_payload(
        ...
        min_count=1,
        expand_radius=1,
        expand_min_token_id=50,
        expand_max_token_id=101,
    )

    assert payload["token_ids"] == [7, 99, 100, 101]
    assert payload["base_token_count"] == 2
    assert payload["expanded_token_count"] == 2
    assert payload["expand_radius"] == 1
    assert payload["counts"] == {"7": 1, "100": 2}
